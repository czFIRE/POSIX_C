...
#define SECCOMP_ADD(...)                                \
    ...
    SECCOMP_ADD(SCMP_ACT_ERRNO(EPERM), SCMP_SYS(rmdir), 0);
    SECCOMP_ADD(SCMP_ACT_ERRNO(EPERM), SCMP_SYS(truncate), 0);

    // network operations
    SECCOMP_ADD(SCMP_ACT_ERRNO(EPERM), SCMP_SYS(bind), 0);
    SECCOMP_ADD(SCMP_ACT_ERRNO(EPERM), SCMP_SYS(connect), 0);
    SECCOMP_ADD(SCMP_ACT_ERRNO(EPERM), SCMP_SYS(sendto), 0);
    SECCOMP_ADD(SCMP_ACT_ERRNO(EPERM), SCMP_SYS(socket), 0);
